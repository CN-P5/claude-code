...
export function AdvisorMessage({
  ...
  switch (block.content.type) {
    case 'advisor_tool_result_error':
      body = <Text color="error">Advisor 不可用（{block.content.error_code}）</Text>;
      break;
    case 'advisor_result':
      body = verbose ? (
        ...
        <Text dimColor>
          {figures.tick} Advisor 已审阅对话并将应用反馈 <CtrlOToExpand />
        </Text>
      );
      break;
    case 'advisor_redacted_result':
      body = <Text dimColor>{figures.tick} Advisor 已审阅对话并将应用反馈</Text>;
      break;
  }
