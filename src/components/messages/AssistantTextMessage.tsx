  ...
  return (
    <MessageResponse>
      <Box flexDirection="column">
        ...
        {isKeychainLocked && <Text dimColor>· 请在另一个终端运行：security unlock-keychain</Text>}
      </Box>
    </MessageResponse>
  );
}
...
export function AssistantTextMessage({
  ...
  switch (text) {
    ...
    case PROMPT_TOO_LONG_ERROR_MESSAGE: {
      ...
      return (
        <MessageResponse height={1}>
          <Text color="error">
            已达到上下文上限 · 输入 /compact 或 /clear 继续
    ...
    case CREDIT_BALANCE_TOO_LOW_ERROR_MESSAGE:
      return (
        <MessageResponse height={1}>
          <Text color="error">账户余额不足 · 充值：https://platform.claude.com/settings/billing</Text>
        </MessageResponse>
      );
    ...
    case API_TIMEOUT_ERROR_MESSAGE:
      return (
        <MessageResponse height={1}>
          <Text color="error">
            {API_TIMEOUT_ERROR_MESSAGE}
            {process.env.API_TIMEOUT_MS && <> (API_TIMEOUT_MS={process.env.API_TIMEOUT_MS} 毫秒，可尝试调大该值)</>}
          </Text>
        </MessageResponse>
      );
      ...
      return (
        <MessageResponse>
          <Box flexDirection="column" gap={1}>
            <Text color="error">Opus 4 目前需求量较大。</Text>
            <Text>如需立即继续，请使用 /model 切换到 {renderModelName(getDefaultSonnetModel())} 并继续编码。</Text>
          </Box>
        </MessageResponse>
      );
