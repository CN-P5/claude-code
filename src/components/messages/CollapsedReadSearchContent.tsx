...
export function CollapsedReadSearchContent({
  ...
  if (isFullscreenEnvEnabled() && message.pushes?.length) {
    ...
    pushPart('push', '已推送至', <Text bold>{branches.join(', ')}</Text>);
  }
  if (isFullscreenEnvEnabled() && message.branches?.length) {
    const byAction = { merged: '已合并', rebased: '已变基至' };
  ...
  if (searchCount > 0) {
    const isFirst = nonMemParts.length === 0;
    const searchVerb = isActiveGroup ? (isFirst ? '正在搜索' : '正在搜索') : isFirst ? '已搜索' : '已搜索';
    if (!isFirst) {
  ...
  if (readCount > 0) {
    const isFirst = nonMemParts.length === 0;
    const readVerb = isActiveGroup ? (isFirst ? '正在读取' : '正在读取') : isFirst ? '已读取' : '已读取';
    if (!isFirst) {
  ...
  if (listCount > 0) {
    const isFirst = nonMemParts.length === 0;
    const listVerb = isActiveGroup ? (isFirst ? '正在列出' : '正在列出') : isFirst ? '已列出' : '已列出';
    if (!isFirst) {
  ...
  if (mcpCallCount > 0) {
    ...
    const isFirst = nonMemParts.length === 0;
    const verb = isActiveGroup ? (isFirst ? '正在查询' : '正在查询') : isFirst ? '已查询' : '已查询';
    if (!isFirst) {
  ...
  if (isFullscreenEnvEnabled() && bashCount > 0) {
    const isFirst = nonMemParts.length === 0;
    const verb = isActiveGroup ? (isFirst ? '正在运行' : '正在运行') : isFirst ? '已运行' : '已运行';
    if (!isFirst) {
      nonMemParts.push(<Text key="comma-bash">, </Text>);
    }
    nonMemParts.push(
      <Text key="bash">
        {verb} <Text bold>{bashCount}</Text> 个 bash 命令
      </Text>,
    );
  }
  ...
  if (memoryReadCount > 0) {
    ...
    const verb = isActiveGroup ? (isFirst ? '正在回忆' : '正在回忆') : isFirst ? '已回忆' : '已回忆';
    if (!isFirst) {
      memParts.push(<Text key="comma-mr">, </Text>);
    }
    memParts.push(
      <Text key="mem-read">
        {verb} <Text bold>{memoryReadCount}</Text> 条记忆
      </Text>,
    );
  }
  ...
  if (memorySearchCount > 0) {
    const isFirst = !hasPrecedingNonMem && memParts.length === 0;
    const verb = isActiveGroup ? (isFirst ? '正在搜索' : '正在搜索') : isFirst ? '已搜索' : '已搜索';
    if (!isFirst) {
  ...
  if (memoryWriteCount > 0) {
    const isFirst = !hasPrecedingNonMem && memParts.length === 0;
    const verb = isActiveGroup ? (isFirst ? '正在写入' : '正在写入') : isFirst ? '已写入' : '已写入';
    if (!isFirst) {
      memParts.push(<Text key="comma-mw">, </Text>);
    }
    memParts.push(
      <Text key="mem-write">
        {verb} <Text bold>{memoryWriteCount}</Text> 条记忆
      </Text>,
    );
  }
